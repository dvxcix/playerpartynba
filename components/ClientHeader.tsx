...
/* =========================
SPIKE FILTER (NEW LOGIC)
========================= */

function spikeAllowed(market: string, anchor: number, spike: number) {

  const diff = spike - anchor;
  const m = market.toLowerCase();

  if (m.includes('points')) return diff <= 10;
  if (m.includes('threes')) return diff <= 2.5;
  if (m.includes('rebounds')) return diff <= 5;
  if (m.includes('assists')) return diff <= 2.5;
  ...
  useEffect(() => {
    ...
    fetch('/api/odds/latest')
      ...
      .then(data => {
        ...
        const anchors = rows.filter((r: PPPRow) =>
          r.over_price === -114 && r.under_price === -114
        );

        const spikes: PPPRow[] = [];

        anchors.forEach((anchor: PPPRow) => {

          rows.forEach((r: PPPRow) => {
            ...
            if (
              ...
              r.line > anchor.line &&
              r.over_price >= 200 &&
              spikeAllowed(anchor.market_name, anchor.line, r.line)
            ) {
              spikes.push(r);
            }
        ...
        spikes.sort((a, b) => a.game.localeCompare(b.game));
                ...
                <table className="table">
                  ...
                  <tbody>
                    ...
                    {pppRows.map((r, i) => {
                      ...
                      return (
                        <tr key={i} onClick={() => scrollToKey(key)}>
